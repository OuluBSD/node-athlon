...
def get_gas_version(cc):
  ...
  warn('Could not recognize `gas`: {}'.format(gas_ret))
  return '0.0'
...
def get_openssl_version(o):
  ...
  try:
    ...
    warn('Failed to determine OpenSSL version from header: {}'.format(e))
    return 0
...
def check_compiler(o):
  ...
  if not ok:
    ...
    warn('C compiler (CC={}, {}) too old, need gcc 4.2 or clang 3.2'.format(CC, version_str))
...
def configure_node(o):
    ...
    if options.enable_pgo_generate or options.enable_pgo_use:
      ...
      if not gcc_version_ge(version_checked):
        ...
        raise Exception(
          ...
          'are supported for gcc and gxx {} or newer only.'.format(version_checked_str))
  ...
  if options.enable_lto:
    ...
    if not gcc_version_ge(gcc_version_checked) and not clang_version_ge(clang_version_checked):
      ...
      raise Exception(
        'The option --enable-lto is supported for gcc {}+'
        'or clang {}+ only.'.format(gcc_version_checked_str, clang_version_checked_str))
...
def configure_library(lib, output, pkgname=None):
  ...
  if getattr(options, shared_lib):
    ...
    default_libs = ['-l{}'.format(l) for l in default_libs.split(',')]
...
def configure_openssl(o):
  ...
  if options.without_ssl:
    def without_ssl_error(option):
      error('--without-ssl is incompatible with {}'.format(option))
...
def configure_sqlite(o):
  ...
  if options.without_sqlite:
    def without_sqlite_error(option):
      error('--without-sqlite is incompatible with {}'.format(option))
...
def write(filename, data):
  print_verbose('creating {}'.format(filename))
...
def glob_to_var(dir_base, dir_sub, patch_dir):
  file_list = []
  dir_all = '{}/{}'.format(dir_base, dir_sub)
  ...
  for ent in files:
    ...
    for file in files:
      if file.endswith(('.cpp', '.c', '.h')):
        ...
        srcfile = '{}/{}'.format(dir_sub, file)
        if patch_dir:
          patchfile = Path(dir_base, patch_dir, file)
          if patchfile.is_file():
            srcfile = '{}/{}'.format(patch_dir, file)
            info('Using floating patch "{}" from "{}"'.format(patchfile, dir_base))
...
def configure_intl(o):
    ...
    for icu in icus:
      ...
      if not expectHash:
        error('''Could not find a hash to verify ICU download.
          {} may be incorrect.
          For the entry {},
          Expected one of these keys: {}'''.format(depFile, url, ' '.join(allAlgos)))
      local = url.split('/')[-1]
      ...
      if not targetfile.is_file():
        ...
        print('Re-using existing {}'.format(targetfile))
      if targetfile.is_file():
        print('Checking file integrity with {}:\r'.format(hashAlgo))
        gotHash = nodedownload.checkHash(targetfile, hashAlgo)
        print('{}:      {}  {}'.format(hashAlgo, gotHash, targetfile))
        if expectHash == gotHash:
          return targetfile

        warn('Expected: {}      *MISMATCH*'.format(expectHash))
        warn('\n ** Corrupted ZIP? Delete {} to retry download.\n'.format(targetfile))
    return None
  icu_config = {
    'variables': {}
  }
  ...
  if canned_is_small:
    warn('Ignoring {} - in-repo small icu is no longer supported.'.format(canned_icu_dir))
  ...
  # now, check that they didn't pass --with-icu-source=deps/icu
    ...
    warn('Ignoring redundant --with-icu-source={}'.format(with_icu_source))
    with_icu_source = None
  # if with_icu_source is still set, try to use it.
  if with_icu_source:
    if Path(icu_full_path).is_dir():
      print('Deleting old ICU source: {}'.format(icu_full_path))
      shutil.rmtree(icu_full_path)
    # now, what path was given?
    if Path(with_icu_source).is_dir():
      # it's a path. Copy it.
      print('{} -> {}'.format(with_icu_source, icu_full_path))
      ...
      if tmp_icu.is_dir():
        ...
        shutil.rmtree(icu_tmp_path)
        error('--with-icu-source={} did not result in an "icu" dir.'.format(with_icu_source))
  ...
  if not Path(icu_full_path).is_dir():
    error('''Cannot build Intl without ICU in {}.
       Fix, or disable with "--with-intl=none"'''.format(icu_full_path))
  else:
    print_verbose('* Using ICU in {}'.format(icu_full_path))
  # Now, what version of ICU is it? We just need the "major", such as 54.
  # uvernum.h contains it as a #define.
  uvernum_h = Path(icu_full_path, 'source', 'common', 'unicode', 'uvernum.h')
  if not uvernum_h.is_file():
    error('Could not load {} - is ICU installed?'.format(uvernum_h))
  icu_ver_major = None
  matchVerExp = r'^\s*#define\s+U_ICU_VERSION_SHORT\s+"([^"]*)".*'
  match_version = re.compile(matchVerExp)
  with io.open(str(uvernum_h), encoding='utf8') as in_file:
    for line in in_file:
      m = match_version.match(line)
      if m:
        icu_ver_major = str(m.group(1))
  if not icu_ver_major:
    error('Could not read U_ICU_VERSION_SHORT version from {}'.format(uvernum_h))
  ...
  if not icu_data_path.is_file():
    ...
    error('''ICU prebuilt data file {} does not exist.
       See the README.md.'''.format(icu_data_path))
  ...
  for key, value in icu_src.items():
    var  = 'icu_src_{}'.format(key)
    path = '../../{}/source/{}'.format(icu_full_path, value)
    icu_config['variables'][var] = glob_to_var('tools/icu', path, 'patches/{}/source/{}'.format(icu_ver_major, value))
