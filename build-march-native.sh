...
if [ "$WITH_INTL" = false ]; then
  ...
  echo "Configuring build with native architecture optimizations (without internationalization)..."
else
  echo "Configuring build with native architecture optimizations (with internationalization)..."
fi
